...
class LoginRequest extends FormRequest
{
    ...
    public function ensureIsNotRateLimited(): void
    {
        ...
        if (! RateLimiter::tooManyAttempts($this->throttleKey(), 5)) {
            Log::info('Login request is not rate limited');
            return;
        }

        event(new Lockout($this));

        Log::info('Login request is rate limited');
    ...
    {
        Log::info('Throttle key: ' . Str::transliterate(Str::lower($this->string('dni')) . '|' . $this->ip()));
